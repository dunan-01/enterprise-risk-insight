...
流程（V3.0）：
  ...
  → 风险等级确定
  → 输出结构化认定结果（own_risk + related_risk_facts）
"""
...
# ------------------------------------------------------------
# Evidence Provenance: 关系图构建与路径查找
# ------------------------------------------------------------
...
class RiskRuleEngine:
    ...
    输出：
        {
            ...
            "risk_level": str,               # 风险等级（低风险/中风险/高风险/重大风险）
    ...
    def evaluate(
        ...
        返回：
            结构化评估结果 dict（含 own_risk + related_risk_facts）。
        """
        extra = extra or {}
        ...
        # max_hits 按 (rule_id, owner_company_id) 计数，避免跨公司共享配额
        ...
        for fact in evidence_facts:
            ...
            for rule in evidence_level_rules:
                ...
                # max_hits 上限检查（per owner_company_id）
        ...
        for owner_cid, company_facts in company_evidence_map.items():
            ...
            # Company-level 规则（使用 extra）仅对目标企业自身评估
            ...
            for rule in company_level_rules:
                ...
                if _eval_condition(rule["condition"], context):
                    ...
                    all_triggered_rules.append({
                        ...
                        # company-level 规则记录所有支撑证据
                        "supporting_evidence_ids": supporting_eids,
                        "hit_count": 1,  # 公司级规则每个公司只命中一次
                    })

        # --------------------------------------------------------
        # 4. 按 provenance 分离 own / related triggered rules
        # --------------------------------------------------------
        ...
        return {
            # 兼容字段（保留，值等于 own_risk）
            "risk_score": own_score,
            "risk_level": own_level,
            "triggered_rules": own_triggered,  # 兼容：只返回 own triggered
            ...
            # 关联企业风险事实（不计算传导分数）
            "related_risk_facts": self._build_related_risk_facts(evidence_facts),
        }
    ...
    def _build_related_risk_facts(self, evidence_facts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """构建关联企业风险事实（不计算传导分数）。
        ...
        related_facts = []
        for fact in evidence_facts:
            if not fact.get("is_target_company", True):
                related_facts.append({
                    "company_id": fact.get("owner_company_id", ""),
                    "evidence_id": fact.get("evidence_id", ""),
                    "evidence_type": fact.get("evidence_type", ""),
                    "description": fact.get("data", {}).get("description", ""),
                })
